...
class Request {
     ...
     * 服务器IP地址
     */
//    val baseUrl: String = "10.129.47.131"
    ...
    private fun createClient(): HttpClient {
        return HttpClient(CIO) {
            engine {
                requestTimeout = 60_000 // 请求超时：15秒
            }
        }
    }
...
//            return ""
            ...
            var result = jsonObject.getString("result").lowercase()
            if (result == "null") {
                result = "male"
            }
    ...
    suspend fun testGender(id: String, gender: String) {
        try {

            @Serializable
            data class GenderData(val id: String,val result: String)
            ...
            val port = 10010
            val url = "http://$baseUrl:$port/gender"

            val genderData = GenderData(id, gender)

            val response: HttpResponse = client.post(url) {
                contentType(ContentType.Application.Json)  // 设置请求内容类型
                setBody(genderData)  // 发送 JSON 数据
            }.body()
        } catch (e: Exception) {
            Log.e("exception", "错误: ${e.localizedMessage}")
        }
